...
erDiagram
    ...
    c_item_code_master {
        ...
        string c_category "C項目大分類"
        string c_subcategory "C項目小分類"
        int score "該当スコア（通常1）"
        date effective_from
        date effective_to
    }
    ...
    c_item_code_master ||--o{ daily_nursing_evaluation : "C項目コードが評価レコードに紐付く"
...
| c_item_code_master | レセプト電算コード↔C項目分類の対応 | C項目の自動判定に使用 |
...
**C項目の保存方針**として、判定結果（0 or 1）のみ `daily_nursing_evaluation` に保存する。判定の根拠となるレセプト電算コードは `c_receipt_code` として合わせて保存し、`c_item_code_master` との照合で分類を確認できる。
