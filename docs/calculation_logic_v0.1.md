...
| 0.6 | 2026年3月 | Step 8 看護必要度詳細画面の実装を反映。列並び順（患者基本情報→得点・基準→個別項目）、項目名フル表示、基準条件式明記、病棟名称列追加、Excel出力同期、生成プログレス6ステップ化 |
...
`GenIIDailyScore[]` をもとに、レコード詳細画面の「看護必要度詳細」タブに**一覧テーブル**を表示する。  
他社製品の「詳細データ（Ⅰ・Ⅱ比較）」画面に相当する機能であり、管理者がスコアの一覧を確認しフィルタリング・Excel出力できるようにする。

### 6-2. テーブル列構成

他社スクリーンショットおよび `GenIIDailyScore` の構造を参考に、以下のカラム構成とする。
全ての個別項目を初期表示で出力し、ヘッダには記号だけでなく**項目名称**もわかるようにする。
...
|-----------|-----------|--------|
| # | 行番号（表示用） | 1, 2, 3... |
| 病棟コード | `wardCode` | GEKA01 |
| データ識別番号 | `patientNo` | 0000000151 |
| 病棟コード | `wardCode` | — |
| 病棟名称 | — | `wardCode` をレコードの `ward_setting` から変換表示（props経由） |
| データ識別番号 | `patientNo` | — |
| 退院年月日 | `dischargeDate` | — |
| 入院年月日 | `admissionDate` | — |
| 実施年月日 | `evalDate` | — |
...
| B得点 | `bTotal` | — |
| C得点 | `cTotal` | — |
| 基準① A≧３ or C≧１ | `meetsP1` | ○/— で表示 |
| 基準② A≧２かつB≧３ or A≧３ or C≧１ | `meetsP2` | ○/— で表示 |
| 基準③ A≧１ or C≧１ | `meetsP3` | ○/— で表示 |
...
> ※ A/B/C個別項目のヘッダは記号（A1、B2等）を使わず、**項目名称のみ**を表示する。  
> ※ 列幅は `max-w-[5.5rem]` で制限し、2〜3行の折り返しを許容する。  
> ※ 病棟名称列は `GenIIDailyScore` 型には存在せず、レコードの `ward_setting` から `wardCode → wardName` マッピングをpropsで渡して変換表示する。

### 6-3. UI仕様

#### テーブル本体
- **ページネーション**: 100件単位でページ分割（既存 `DebugTab` と同様の方式）
- **ソート**: 初期状態は患者番号×日付の昇順（`convertScoreMapToArray` で事前ソート済み）
- **フィルタ**: ヘッダ行にフィルタ入力を設置
  - 病棟（病棟名称のドロップダウンで選択）
  - データ識別番号（テキスト部分一致）
- **条件付き書式**: `meetsP1/P2/P3` が `true` の行は背景色をハイライト（参考: 他社は水色・ピンクで色分け）
- **件数表示**: テーブル上部に「○○ 件」と全件数を表示

#### Excel出力
- **ボタン**: テーブル右上に「Excel出力」ボタンを配置
- **ライブラリ**: `xlsx`（SheetJS）を使用
- **出力内容**: テーブルに表示されている全データ（フィルタ適用時はフィルタ後のデータ）
- **ファイル名**: `看護必要度詳細_{レコードタイトル}_{YYYYMMDD}.xlsx`
- **シート構成**: 1シート（「看護必要度詳細」）
...
├── NursingDetailTab.tsx      … タブ本体（テーブル + フィルタ + ページネーション）

src/lib/
└── export-to-excel.ts        … Excel出力ユーティリティ関数
```

- `NursingDetailTab` は Zustand Store の `dailyScores` を参照して表示する
- `export-to-excel.ts` は `GenIIDailyScore[]` を受け取り、xlsxファイルを生成・ダウンロードする

### 6-5. 処理シーケンス

```
1. ユーザーが「看護必要度詳細」タブを選択
2. NursingDetailTab が Store から dailyScores を取得
3. フィルタ条件に基づいてフィルタリング（クライアント側、リアルタイム）
4. ページネーションに基づいて表示範囲を決定
5. テーブルをレンダリング
6.「Excel出力」ボタン押下時:
   a. フィルタ適用後の GenIIDailyScore[] を export-to-excel に渡す
   b. xlsx ファイルをブラウザからダウンロード
```
